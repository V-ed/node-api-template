...
export class SocketIoAdapter extends AbstractWsAdapter {
	constructor(appOrHttpServer?: INestApplicationContext | any, private serverOptions?: Partial<ServerOptions>) {
		super(appOrHttpServer);
	}

	public create(port: number, options?: { namespace?: string; server?: Server; [key: string]: unknown }): any {
		if (!options) {
			return this.createIOServer(port);
		}
		const { namespace, server, ...opt } = options;

		return server && isFunction(server.of) && namespace
			? server.of(namespace)
			: namespace
	...
	public createIOServer(port: number, options?: any): Server {
		if (this.httpServer && port === 0) {
			return new Server(this.httpServer, this.serverOptions);
		}
